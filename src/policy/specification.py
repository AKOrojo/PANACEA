...
def assign_policies_randomly(mapping_results, field_names, security_metadata_variations, policy_variations):
    """
    Assigns varying security metadata and policies randomly to each URP, regardless of their tree structure.

    :param mapping_results: The list of mappings.
    :param field_names: A single field name or a list of field names.
    :param security_metadata_variations: A list of dictionaries for security metadata.
    :param policy_variations: A list of lists of dictionaries for policies.
    :return: The updated mapping_results with randomly applied policies and metadata.
    """
    if isinstance(field_names, str):
        field_names = [field_names]
